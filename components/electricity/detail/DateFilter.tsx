...
export function buildRange(preset: Exclude<DatePreset, "custom">): DateRange {
  ...
  switch (preset) {
    ...
    case "all":
      return {
        preset,
        from: new Date("2025-01-01T00:00:00").toISOString(),
        to: eod.toISOString(),
      };
    case "today":
    default:
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          size="sm"
          className={cn(
            "gap-1 border-border/50 bg-muted/50 font-normal cursor-pointer",
            isXs ? "h-6 text-[10px] px-2" : "h-7 text-sm px-3 gap-1.5",
          )}
        >
          <CalendarDays
            className={cn(
              isXs ? "h-3 w-3" : "h-3.5 w-3.5",
