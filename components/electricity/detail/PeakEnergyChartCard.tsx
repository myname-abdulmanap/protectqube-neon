...
const buildEnergySeries = (
  ...
  const allKeys = Array.from(bucketMax.keys()).sort((a, b) =>
    a.localeCompare(b),
  );
    ...
    if (perHour) {
      ...
      if (deltaMap.has(targetHourKey)) {
        deltaMap.set(
          ...
          Number(
            (
              (deltaMap.get(targetHourKey) ?? 0) +
              CALIBRATION_OVERRIDE_KWH_DELTA
            ).toFixed(3),
          ),
        );
      }
      ...
      deltaMap.set(
        ...
        Number(
          (
            (deltaMap.get(CALIBRATION_OVERRIDE_DAY_KEY) ?? 0) +
            CALIBRATION_OVERRIDE_KWH_DELTA
          ).toFixed(3),
        ),
      );
  ...
  const toKey = perHour
    ? `${toP.year}-${toP.month}-${toP.day} ${toP.hour}`
  ...
  const spansDays =
    ...
    firstInRange &&
    lastInRange &&
    firstInRange.slice(0, 10) !== lastInRange.slice(0, 10);
...
export function PeakEnergyChartCard({
  scopeId,
  timeSeries,
  loadedFrom,
  loadedTo,
}: PeakEnergyChartCardProps) {
  ...
  const peakPoint = useMemo(
    () =>
      series.length
        ? series.reduce((best, p) => (p.value > best.value ? p : best))
        : null,
    [series],
  );
  const minPoint = useMemo(
    () =>
      series.length
        ? series.reduce((low, p) => (p.value < low.value ? p : low))
        : null,
    [series],
  );
  const totalEnergy = useMemo(
    () => Number(series.reduce((s, p) => s + p.value, 0).toFixed(3)),
    [series],
  );
      ...
      <CardContent className="px-5 pb-5 pt-4 flex flex-col flex-1 gap-4">
        ...
        <div className="grid grid-cols-3 gap-2 shrink-0">
          ...
          <div className="rounded-xl bg-purple-500 flex flex-col px-4 py-3.5">
            ...
            <div className="flex items-baseline gap-1 flex-wrap">
              <p className="text-2xl font-bold tabular-nums text-white leading-none">
                {avgEnergy.toLocaleString("id-ID", {
                  maximumFractionDigits: 3,
                })}
              </p>
          ...
          <div className="rounded-xl bg-teal-500 flex flex-col px-4 py-3.5">
            ...
            <div className="flex items-baseline gap-1 flex-wrap">
              <p className="text-2xl font-bold tabular-nums text-white leading-none">
                {minPoint
                  ? minPoint.value.toLocaleString("id-ID", {
                      maximumFractionDigits: 3,
                    })
                  : "–"}
              </p>
