...
import { ChevronLeft, ChevronRight, Clock, TrendingUp, BarChart2, CalendarDays, Zap, BarChart3 } from 'lucide-react';
...
function buildPageRange(pageOffset: number): { from: string; to: string; label: string } {
	...
	const toDay = new Date(todayWib.getTime() - pageOffset * DAYS_PER_PAGE * 24 * 60 * 60 * 1000);
	const fromDay = new Date(toDay.getTime() - (DAYS_PER_PAGE - 1) * 24 * 60 * 60 * 1000);

	const fmt = new Intl.DateTimeFormat('id-ID', { day: '2-digit', month: 'short', timeZone: DISPLAY_TIMEZONE });
...
function DayChart({ day }: { day: HourlyDailyEnergyDay }) {
	...
	const widestTick = ticks.map(fmtY).reduce((a, b) => (a.length >= b.length ? a : b), '');
		...
		<Card className='border border-border/50 bg-card/60 gap-0 py-0'>
			...
			<CardContent className='px-2 pb-3 pt-1'>
				<ResponsiveContainer width='100%' height={115}>
					<BarChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: leftMargin }} barSize={5}>
						<CartesianGrid strokeDasharray='2 2' vertical={false} opacity={0.15} />
						<XAxis
							dataKey='label'
...
function derivePeriodStats(
	...
	const busiestDay = daysWithData.length
		...
		: null;
	const quietestDay =
		daysWithData.length > 1 ? daysWithData.reduce((low, d) => (d.totalKwh < low.totalKwh ? d : low)) : null;
	const totalKwh = days.reduce((s, d) => s + d.totalKwh, 0);
	return { peakHourLabel: metaPeakHourLabel, avgDailyKwh: metaAvgDailyKwh, totalKwh, busiestDay, quietestDay };
}
...
export function HourlyEnergyCard({ scopeId }: HourlyEnergyCardProps) {
	...
	const fetchData = useCallback(
		async (offset: number) => {
			...
			try {
				...
				const res = await energyDashboardApi.getHourlyDailyEnergy(scopeId, r.from, r.to);
	...
	const stats = useMemo(
		() => derivePeriodStats(days, meta?.peakHourLabel ?? null, meta?.avgDailyKwh ?? 0),
		[days, meta],
	);
			...
			<CardHeader className='px-5 pt-5 pb-0 gap-0 shrink-0'>
				<div className='flex items-center justify-between flex-wrap gap-3'>
					<div className='flex items-center gap-2'>
						...
						<CardTitle className='text-lg font-bold'>Hourly Energy Consumption</CardTitle>
					</div>
				...
				{loading ? (
					<div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
						{Array.from({ length: 6 }).map((_, i) => (
							<div key={i} className='rounded-xl bg-muted/30 animate-pulse h-48' />
						))}
					</div>
				) : days.length === 0 ? (
