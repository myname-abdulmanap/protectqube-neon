...
function DayChart({ day, scopeId }: { day: HourlyDailyEnergyDay; scopeId: string }) {
	const isCalibrationDay = scopeId === CALIBRATION_OVERRIDE_SCOPE_ID && day.date === CALIBRATION_OVERRIDE_DAY_KEY;
	...
	const hasAnyData = day.hours.some((h) => h.hasData);
	...
	return (
		<Card className='border border-border/50 bg-card/60 gap-0 py-0'>
			...
			<CardContent className='px-2 pb-3 pt-1'>
				{!hasAnyData ? (
					<div className='relative h-28.75 w-full flex flex-col items-center justify-center rounded-md overflow-hidden bg-muted/5'>
						<div
