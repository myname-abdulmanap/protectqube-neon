...
import { energyConfigsApi, scopesApi, type EnergyConfig, type Scope } from "@/lib/api";
...
function renderTariffSummary(config: EnergyConfig) {
	...
	if (!tariff || tariff.mode === "flat") {
		...
		return <span className="text-xs font-medium">Flat {formatCurrency(flatPrice)}/kWh</span>;
	}

	if (!tariff.touPeriods?.length) {
		return <span className="text-xs text-muted-foreground">TOU periods not set</span>;
	}

	return (
		<div className="space-y-1">
			{tariff.touPeriods.map((period) => (
				<div key={period.id ?? `${period.label}-${period.startTime}-${period.endTime}`} className="text-xs">
					<span className="font-medium">{period.label}</span>
					<span className="text-muted-foreground"> {period.startTime}-{period.endTime}</span>
...
export default function EnergyConfigHistoryPage() {
	...
	if (!canRead) {
		return <div className="p-4 text-sm text-muted-foreground">No permission</div>;
	}
			...
			<div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
				...
				<Card>
					<CardContent className="p-4">
						...
						<p className="mt-1 text-xl font-semibold text-sky-700">{totals.flatCount}</p>
					</CardContent>
				</Card>
				<Card>
					<CardContent className="p-4">
						<p className="text-xs text-muted-foreground">TOU</p>
						<p className="mt-1 text-xl font-semibold text-amber-700">{totals.touCount}</p>
					</CardContent>
				</Card>
			</div>
									...
									return (
										<TableRow key={config.id}>
											<TableCell className="text-xs font-medium">
												...
												<div className="text-[11px] text-muted-foreground">{config.scopeId}</div>
											</TableCell>
											<TableCell>
												<Badge
													variant="outline"
													className={mode === "tou" ? "border-amber-300 text-amber-700" : "border-sky-300 text-sky-700"}
												>
													{mode.toUpperCase()}
												</Badge>
											</TableCell>
											<TableCell className="min-w-[260px]">{renderTariffSummary(config)}</TableCell>
											<TableCell className="text-xs font-mono">
												{formatCurrency(config.pricePerKwh)}
											</TableCell>
											<TableCell className="text-xs">{config.maxLoadKw ?? "-"}</TableCell>
											<TableCell className="text-xs">{config.capacityVa ?? "-"}</TableCell>
											<TableCell className="text-xs">{config.upperLimitKwh ?? "-"}</TableCell>
