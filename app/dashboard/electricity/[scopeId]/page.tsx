...
export default function ElectricityOutletDetailPage() {
  ...
  const handleExportProcessed = async (
    ...
    const toDateInclusive = new Date(
      toDate.getTime() +
        (useHourInterval ? 60 * 60 * 1000 : 24 * 60 * 60 * 1000),
    );
    ...
    const hourlyDays =
      hourlyRes.success && hourlyRes.data ? hourlyRes.data.days : [];
    ...
    const resolveSlot = (acc: SlotAcc) => ({
      ...
      avgKvarh:
        acc.kvarhLast !== null ? Number(acc.kvarhLast.toFixed(3)) : null,
    });
    ...
    for (const day of hourlyDays) {
      for (const hour of day.hours) {
        ...
        const ts = new Date(
          `${day.date}T${String(hour.hour).padStart(2, "0")}:00:00+07:00`,
        );
        ...
        hourlyBuckets.push({
          ...
          energyKwh:
            hour.energyKwh > 0 ? Number(hour.energyKwh.toFixed(3)) : null,
    ...
    const validEnergy = primaryBuckets
      .map((b) => b.energyKwh)
      .filter((v): v is number => v !== null && v > 0);
    const totalEnergyKwh = Number(
      validEnergy.reduce((s, v) => s + v, 0).toFixed(3),
    );
    const avgEnergyKwh = Number(
      (validEnergy.length ? totalEnergyKwh / validEnergy.length : 0).toFixed(3),
    );
    ...
    const peakBucket =
      [...primaryBuckets]
        ...
        .reduce<EnergyBucket | null>(
          (best, b) =>
            (b.energyKwh ?? -Infinity) > (best?.energyKwh ?? -Infinity)
              ? b
              : best,
          null,
        ) ??
    ...
    const analyticsAvgKvarh =
      backendKvarh > 0 ? Number(backendKvarh.toFixed(3)) : computedKvarh;
    ...
    const analyticsAvgFreq =
      backendFreq !== null && backendFreq !== undefined
        ? Number(backendFreq.toFixed(2))
        : (computedFreq ?? 0);
    ...
    const analyticsExcelRow = Object.fromEntries(analyticsRows) as Record<
      string,
      string | number
    >;
  ...
  const handleExportRaw = async (
    ...
    const toDateInclusive = new Date(
      new Date(normalizeExportPeriod(period).toIso).getTime() +
        24 * 60 * 60 * 1000,
    );
