...
import { deviceMetricsApi, energyConfigsApi, energyDashboardApi } from '@/lib/api';
...
const getLatestMetricsByKey = (metrics: MetricItem[]) => {
	...
	for (const m of metrics) {
		...
		if (!cur || new Date(m.timestamp) > new Date(cur.timestamp)) map.set(m.metricKey, m);
	}
	return map;
};
...
export default function ElectricityOutletsPage() {
			...
			const merged = await Promise.all(
				summaryResponse.data.map(async (summary) => {
					...
					const statusLabel: 'online' | 'warning' | 'offline' =
						summary.status === 'alert' ? 'warning' : summary.status === 'normal' ? 'online' : 'offline';
					...
					const toVal = (key: string) => Number(latestByKey.get(key)?.metricValue ?? 0);
					...
					const latestConfig =
						configResponse.success && configResponse.data && configResponse.data.length > 0
					...
					const energyMonth = applyStartPointOffset(rawEnergy, startPoint, energyMetric?.timestamp);
	...
	const filteredOutlets = useMemo(() => {
		...
		return outlets.filter((o) => o.name.toLowerCase().includes(kw) || o.region.toLowerCase().includes(kw));
	}, [outlets, search]);

	return (
		<PageTransition>
			<motion.div className='p-5 space-y-6' initial='hidden' animate='visible' variants={containerVariants}>
				<motion.div
							...
							<Table>
								<TableHeader>
									<TableRow className='hover:bg-transparent bg-muted/30 dark:bg-muted/20'>
										<TableHead className='whitespace-nowrap px-4 py-3'>Outlet</TableHead>
										<TableHead className='whitespace-nowrap px-4 py-3'>Region</TableHead>
										...
										filteredOutlets.map((outlet, idx) => {
											...
											return (
												<motion.tr
													...
													onClick={() =>
														router.push(`/dashboard/electricity/${outlet.scopeId}`)
													}
												>
				...
				{error && (
					<motion.p variants={itemVariants} className='text-[11px] text-destructive'>
						{error}
					</motion.p>
				)}
			</motion.div>
